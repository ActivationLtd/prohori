...
class PartnersController extends ModulebaseController
{
    /*********************************************************************
    ...
    {
        return [
            // ['partners.id', 'id', 'ID'], // translates to => table.id as id and the last one ID is grid colum header
            ["{$this->module_name}.id", "id", "ID"],
            ["{$this->module_name}.name", "name", "Name"],
            // ["{$this->module_name}.live_url_root", "live_url_root", "Website"],
            ["{$this->module_name}.currency", "currency", "Currency"],
            ["{$this->module_name}.order", "order", "Order"],
            ["{$this->module_name}.partnercategory_name", "partnercategory_name", "Category"],
            ["{$this->module_name}.contact_email", "contact_email", "Contact Email"],
    ...
    public function datatableModify($dt)
    {
        ...
        $dt = $dt->editColumn('currency', '{{$currency}} ({{ curSign($currency)}})');
    ...
    public function editMyPartner()
    {
        return app(PartnersController::class)->edit(user()->partner_id);
    }
