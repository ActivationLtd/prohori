...
$recommendations_count =  \App\Recommendurl::remember(cacheTime('short'))->select(DB::raw('count(id) as recommend_count'),DB::raw('extract(year from created_at) as year'),DB::raw('extract(month from created_at) as month'),DB::raw('CONCAT(extract(year from created_at), extract(month from created_at)) as yearmonth'))->groupBy('month','year')->orderBy('yearmonth')->where("created_at",">", \Carbon\Carbon::now()->subMonths($month))->get()->toArray();
/* Query to get purchases count according to months and year */
$purchases_count =  \App\Purchase::remember(cacheTime('short'))->select(DB::raw('count(id) as purchase_count'),DB::raw('extract(year from created_at) as year'),DB::raw('extract(month from created_at) as month'),DB::raw('CONCAT(extract(year from created_at), extract(month from created_at)) as yearmonth'))->groupBy('month','year')->orderBy('yearmonth')->where("created_at",">", \Carbon\Carbon::now()->subMonths($month))->get()->toArray();
...
<div class="row">
    <div class="col-md-12">
        <h5>Recommendation vs purchases (Last {{ $month }} months)</h5>
