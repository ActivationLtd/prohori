...
@section('js')
    ...
    <script>
        ...
        L.marker([{{$userlocation->latitude}}, {{$userlocation->longitude}}])
            ...
            .bindPopup("<a href='{{route('userlocations.edit',$userlocation->id)}}'><img style='width:50px' src='{{asset($userlocation->user->profile_pic_url)}}'/><b>&nbsp{{$userlocation->user->name}}<br>Distance&nbsp{{$userlocation->distance_flag}} <br>Time&nbsp{{$userlocation->created_at}}</a>", {
                autoClose: false,
                autoPan: false
            })
            .openPopup();
