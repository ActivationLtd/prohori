...
<table id="tbl_purchases_not_invoiced" class="table module-grid table table-bordered table-striped"
    ...
    @foreach($purchases_not_invoiced as $purchase)
        ...
        $donation_css = $purchase->charity_donation_charity_currency > 0 ? 'bg-green disabled color-palette' : '';
        ...
        <tr>
            <td>
                @if($purchase->isInvoicable() && $purchase->charity_donation_charity_currency >0)
            ...
            <td>{{ $purchase->recommender->email }}</td>
            <td class="{{$donation_css}}">{{symbol($purchase->charity_currency)}} {{money($purchase->charity_donation_charity_currency)}}</td>
...
<button id="genereate_invoice_btn" class="btn btn-default" type="submit">Generate invoice</button>
