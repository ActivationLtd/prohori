...
<table id="tbl_purchases_not_invoiced" class="table module-grid table table-bordered table-striped"
    ...
    @foreach($purchases_not_invoiced as $purchase)
        ...
        $commission_css = $purchase->user_commission_in_user_currency > 0 ? 'bg-green disabled color-palette' : '';
        ...
        <tr>
            <td>
                @if($purchase->isInvoicable() && $purchase->user_commission_in_user_currency >0)
            ...
            <td class="{{$commission_css}}">{{symbol($purchase->user_currency)}} {{money($purchase->user_commission_in_user_currency)}}</td>
